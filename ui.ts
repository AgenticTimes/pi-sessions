...
type SessionsActions = {
	...
	/** 渲染某会话的真实聊天组件（LLM 输出 UI），用于光标移动时实时切换预览 */
	getSessionChat?: (id: string, width: number) => string[];
};
...
export class SessionWidget implements Component {
	...
	private fitFromRight(segments: string[], width: number): string {
		...
		// 第一个 segment（listLive 顺序 = 父会话；子会话视图下父会话在首位）
		// 永远钉在左侧，其余从右往左填充——避免子会话一多把父会话挤出状态栏
		...
		const full = line ? `${pinned}  ${line}` : pinned;
		return visibleWidth(full) <= width ? full : pinned;
	}
...
class SessionsView {
	...
	/** 读 transcript 尾部，返回最近消息条目 [{label, text}]（label: user/assistant/tool/think/result） */
	private readTranscriptMessages(file: string): { label: string; text: string }[] {
		...
		try {
			...
			const clean = firstNl >= 0 ? tail.slice(firstNl + 1) : tail;
			...
			for (const line of clean.split("\n")) {
				...
				for (const c of content) {
					...
					if (c.type === "text") {
						...
						label = "tool";
						text = c.name || c.toolName || "call";
					} else if (c.type === "toolResult") {
						label = "result";
						const cc = Array.isArray(c.content) ? c.content : [];
						text = cc[0]?.text ?? "";
					}
	...
	/** 宽度感知换行（CJK 计 2 列），不处理 ANSI */
	private wrapText(text: string, w: number): string[] {
		const out: string[] = [];
		let cur = "";
		let curW = 0;
	...
	/** 顶部主区：优先渲染选中会话的【真实聊天组件】（LLM 输出 UI，取底部可视窗口）；无 live 组件时回退 transcript 重建 */
	private sessionChatLines(
		...
		const muted = (s: string) => th.fg("muted", s);
		const lines: string[] = [];
		const name = session?.shortName || session?.name || "(no session)";
		lines.push(
			fits(
				width,
				`${accent("◆")} ${accent(name)}${session?.cwd ? dim(`  ${session.cwd}`) : ""}`,
			),
		);
		const bodyMax = maxLines - 1;
		if (!session) {
			lines.push(
				muted("  ↑↓ 选择会话，主区实时显示其 LLM 输出界面，enter 切换"),
			);
		...
		let body: string[] = [];
		try {
			body = this.actions.getSessionChat?.(session.id, width) ?? [];
		} catch {}
		if (!body.length) body = this.transcriptFallbackLines(session, width);
		lines.push(...body.slice(-bodyMax));
		while (lines.length < maxLines) lines.push(" ");
		return lines.slice(0, maxLines);
	}

	/** transcript 重建（无 live 聊天组件时的回退） */
	private transcriptFallbackLines(session: SessionInfo, width: number): string[] {
		const th = this.theme;
		const accent = (s: string) => th.fg("accent", s);
		const dim = (s: string) => th.fg("dim", s);
		const muted = (s: string) => th.fg("muted", s);
		if (!session.sessionFile) return [muted("  (transcript unavailable)")];
		const pending: string[] = [];
		for (const { label, text } of this.readTranscriptMessages(
			session.sessionFile,
		).slice(-14)) {
			const content = text.replace(/\n/g, " ");
			if (label === "user") {
				for (const l of this.wrapText(content, width - 4))
					pending.push(padVisible(accent(` ▌ ${l}`), width));
			} else if (label === "assistant") {
				for (const l of this.wrapText(content, width - 4))
					pending.push(padVisible(`   ${l}`, width));
			} else if (label === "tool") {
				pending.push(dim(` ⚙ ${truncateToWidth(content, width - 6, "…")}`));
			} else if (label === "think") {
				pending.push(muted(` … ${truncateToWidth(content, width - 6, "…")}`));
			} else {
				pending.push(muted(` ↳ ${truncateToWidth(content, width - 6, "…")}`));
			}
		}
	...
	render(width: number): string[] {
		...
		lines.push(border());
		// ★ 顶部主区：光标选中会话的【真实 LLM 输出界面】（↑↓ 移动即切换，enter 切换目标）
		lines.push(...this.sessionChatLines(this.selectedSession(), width, 10));
		lines.push(border("dim"));
		lines.push(accent(fits(width, `${prefix}${renderedInput}`)));
		lines.push(border("dim"));
